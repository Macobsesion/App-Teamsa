...
- **Catálogos Limpios**: Cuando se modifiquen atributos de entidades planas (como `Cliente`, `Proveedor`), el AI debe usar la Herencia SQLModel. Las propiedades con `Field(...)` deben residir **una única vez** en el archivo `_esquemas.py` (`class ClienteBase(SQLModel)`). El modelo físico en `_modelo.py` DEBE heredar de dicha clase sin reescribir los atributos.
