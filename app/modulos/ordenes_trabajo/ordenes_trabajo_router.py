...
from app.modulos.ordenes_trabajo.dependencias import obtener_repo_ordenes, obtener_servicio_ordenes
from app.modulos.usuarios.usuarios_esquemas import UsuarioIdentity
from app.modulos.ordenes_trabajo.pdf_generator import generar_pdf_orden
...
class PayloadCrearOT(BaseModel):
    ...
    hora_programada: str
    duracion: int = 1
    unidad_duracion: str = "horas"
    tecnico_id: int | None = None
    concepto_ids: list[int] = []
    fuerza: bool = False # Para ignorar empalmes de horario
...
def crear_desde_cotizacion(
    ...
    servicio: ServicioOrdenes = Depends(obtener_servicio_ordenes),
    usuario: UsuarioIdentity = Depends(para_modulo("ordenes_trabajo", "crear"))
):
...
def completar_concepto(
    ...
    puede_editar = "ordenes_trabajo" in per_edit

    return TEMPLATES.TemplateResponse(
        "ui/ordenes_trabajo/_fila_concepto.html",
        {
            "request": request,
            "concepto": concepto,
...
def reasignar_tecnico(
    ...
    return servicio.reasignar_tecnico(orden_id, payload.tecnico_id, usuario.usuario, fuerza=payload.fuerza)
...
def actualizar_notas_privadas(
    ...
    db: Session = Depends(obtener_sesion_bd),
    repo: RepositorioOrden = Depends(obtener_repo_ordenes),
    usuario: UsuarioIdentity = Depends(para_modulo("ordenes_trabajo")),
):
...
router_ui_extras = APIRouter(prefix="/ui/ordenes-trabajo", tags=["Órdenes de Trabajo - UI"])
...
def ver_detalle_orden(
    ...
    request: Request,
    repo: RepositorioOrden = Depends(obtener_repo_ordenes),
    usuario = Depends(para_modulo("ordenes_trabajo", "ver"))
):
    ...
    puede_editar = "ordenes_trabajo" in perms_edit
    puede_eliminar = "ordenes_trabajo" in perms_delete
